...
## 功能描述
新增後台頁面「前台訂單收件匣」，列出合併後 Sheet 的 `八字蛋糕訂單` / `下午茶訂單` 兩張 tab（前台 GAS 寫入的單），提供「一鍵轉入 orders」功能，把收件匣的單依欄位映射寫入後台 `orders` tab。轉單時允許補填前台沒收集的欄位（最關鍵是**心願主項 / 心願細項**——八字分析的必要輸入）。解決目前「前台訂單與後台 orders 兩條流互不相通、前台單永遠不進後台」的斷點。

## 頁面與路由（建議，最終以 backend/SA 命名為準）
- `GET /admin/inbox`：列出八字蛋糕訂單 + 下午茶訂單未轉單的列，顯示來源 tab、下單時間、客人資訊。
- `POST /admin/inbox/convert`：將指定來源列轉入 orders，附帶師傅補填欄位。

## 欄位映射（八字蛋糕訂單 → orders）
前台 `八字蛋糕訂單`（10 欄）：`時間, name, phone, birthdate, birth_hour, product, quantity, pickup_date, delivery, notes`
後台 `orders`（14 欄）：`id, 建立時間, 姓名, 電話, 生日, 時辰, 心願主項, 心願細項, 備註, 狀態, 八字分析, 選用食材, 小卡文案, 五行建議`

| orders 欄位 | 來源 | 規則 |
|-------------|------|------|
| id | 系統產生 | 沿用 db.py get_next_order_id 規則（見「待確認 D2」） |
| 建立時間 | 轉單當下時間 或 前台「時間」 | 待確認 D3 |
| 姓名 | name | 直接映射 |
| 電話 | phone | 直接映射 |
| 生日 | birthdate | 直接映射 |
| 時辰 | birth_hour | 直接映射 |
| 心願主項 | **師傅補填** | 前台未收集，轉單表單必填（八字分析必要輸入） |
| 心願細項 | **師傅補填** | 前台未收集，轉單表單可選 |
| 備註 | notes（＋ product/quantity/pickup_date/delivery 併入備註） | 待確認 D4：product/quantity/pickup_date/delivery 這些前台有、orders 無對應欄的資料如何保存 |
| 狀態 | 固定「待處理」 | 轉入後進入既有八字分析流程 |
| 八字分析 / 選用食材 / 小卡文案 / 五行建議 | 留空 | 由後台 analyze / generate_card 後續產生 |

> 下午茶訂單（8 欄：`時間, company, contact, phone, items, event_date, total_qty, notes`）的轉單映射：下午茶單**不走八字分析流程**，與 orders（八字蛋糕專用 schema）結構不符。**待確認 D1**：下午茶訂單是否也要轉入 orders？還是收件匣只做「檢視 + 標記已處理」即可？不腦補，列待確認。

## 業務規則
1. 轉單後需標記該前台單為「已轉入」，避免重複轉單（標記方式待確認 D5：在來源 tab 加狀態欄？還是後台記已轉 id？）。
2. 心願主項/心願細項為八字分析必要輸入，八字蛋糕單轉單時「心願主項」必填。
3. 收件匣只讀前台訂單 tab + 寫 orders，不修改前台訂單原始資料（除標記已轉外）。

## 邊界條件 / 錯誤處理
- 同一列被重複轉單 → 需擋下或警告（依 D5 機制）。
- 收件匣 tab 為空 → 顯示「目前無待轉訂單」，不報錯。
- 轉單寫 orders 失敗 → 回明確錯誤，不可標記為已轉。

## 驗收條件（QA 可執行）
1. `/admin/inbox` 正確列出 `八字蛋糕訂單` 的單（以測試資料驗證，不污染正式資料）。
2. 一鍵轉單：補填心願主項後，orders tab 新增一列，14 欄映射正確、狀態為「待處理」。
3. 轉入的單可在 `/admin/order/<id>` 開啟，並能正常跑 `/admin/api/analyze`（心願欄位有值，分析不缺輸入）。
4. 重複轉單被擋下 / 警告。
5. （下午茶處理方式依 D1 結論補驗收）。
...
- 規則：
  ...
  - 備援聯絡方式內容**待確認 D7**（電話？Email？LINE？由 Olina 提供）。
- no-cors 模式無法讀回應狀態的技術限制需在 impact 評估：若無法可靠偵測 GAS 端寫入成功與否，至少要覆蓋「URL 為空」「網路層 reject」兩種可偵測失敗，並在這些情況顯示備援。

## 驗收條件（QA 可執行）
1. 後台品牌頁將 `cta_url_bazi` 清空後存檔 → 出現警告 / 二次確認（依 D6）。
2. 前台 GAS URL 為空時送單 → 客人看到失敗訊息 + 備援聯絡方式（非成功畫面）。
3. 前台網路層送單失敗（模擬）→ 同樣顯示失敗 + 備援。
...
## 4.4 orders tab 標題列修正
- 後台 Sheet `orders` tab 標題列是舊版 18 欄、資料列是新版 14 欄（db.py 跳過標題列按位置讀，程式無感，但人工對標題填資料必錯位）。
- 修正：把標題列改為現行 `ORDERS_HEADER`（db.py 定義的 14 欄）。
- ⚠️OLINA 或 backend：此為 Sheet 內容修改。若 service_account 有寫入權限，backend 可程式修正；否則列為 Olina 手動（手動清單 M4）。**只改標題列，不可動資料列**。
...
| 編號 | 問題 | 影響 Phase |
|------|------|-----------|
| **D1** | 下午茶訂單（8 欄）要轉入 orders 嗎？orders 是八字蛋糕專用 14 欄 schema，下午茶結構不符。或收件匣對下午茶只做「檢視 + 標記已處理」即可？ | Phase 2 |
| **D2** | orders `id` 取號規則：沿用 get_next_order_id（流水號）即可，還是有業務編碼規則（如日期前綴）？ | Phase 2 / 4 |
| **D3** | 轉單後 orders「建立時間」用轉單當下，還是用前台下單時間？ | Phase 2 |
| **D4** | 前台八字蛋糕單有、orders 無對應欄的資料（product / quantity / pickup_date / delivery）如何保存？併入備註？還是 orders 需擴欄？ | Phase 2 |
| **D5** | 防重複轉單的標記方式：在來源 tab 加「已轉入」狀態欄？還是後台記錄已轉來源列？ | Phase 2 |
| **D6** | 品牌頁存空值：警告後仍可強制存（預設），還是禁止存空 cta_url？ | Phase 3 |
| **D7** | 前台送單失敗時顯示的「備援聯絡方式」具體內容（電話 / Email / LINE）？ | Phase 3 |
| **D8** | orders `狀態` 欄完整業務流程：除程式出現的 待處理/分析完成/文案完成，是否有人工狀態（已交付/取消等）？影響收件匣與訂單管理的狀態設計。 | Phase 2（次要） |

> D1–D7 不阻擋 Phase 0；D1/D3/D4/D5 在 Phase 2 開工前需 Olina 回覆；D6/D7 在 Phase 3 開工前需回覆。D2/D8 可並行確認。
